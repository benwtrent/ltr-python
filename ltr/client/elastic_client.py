...
class ElasticClient(BaseClient):
    ...
    def index_documents(self, index, doc_src):
        ...
        resp = elasticsearch.helpers.bulk(
            self.es, bulkDocs(doc_src), chunk_size=100)
        self.es.indices.refresh(index=index)
        resp_msg(msg="Streaming Bulk index DONE {}".format(
            index), resp=BulkResp(resp))

    def reset_ltr(self, index):
        resp = requests.delete(self.elastic_ep)
        resp_msg(msg="Removed Default LTR feature store".format(),
                 resp=resp, throw=False)
    ...
    def submit_model(self, featureset, index, model_name, model_payload):
        ...
        create_ep = "{}/_featureset/{}/_createmodel".format(
            self.elastic_ep, featureset)
